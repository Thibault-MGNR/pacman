...
namespace Game {
    ...
    Texture::Texture(std::shared_ptr<Renderer> renderer, std::string path) : _renderer(renderer), _path(path){
        this->initialize();
    }

    Texture::Texture(std::shared_ptr<Renderer> renderer, std::string path, Texture_data data) : _renderer(renderer), _path(path), _data(data){
        this->initialize();
    }

    std::shared_ptr<SDL_Texture> Texture::get_texture() const noexcept{
